...
[F1,F2,sol] = findFundamentalMatrix(x1,x2,x3,x4,x5,x6,x7,x1n,x2n,x3n,x4n,x5n,x6n,x7n);
inliera = -1; inlierb = -1; inlierc = -1;
    if isreal(sol(1))
    Fa = F1 + sol(1)*F2;
    inliera = findInlierCount(Fa,matchedPoints1,matchedPoints2);
    end
    if isreal(sol(2))
    Fb = F1 + sol(2)*F2;
    inlierb = findInlierCount(Fb,matchedPoints1,matchedPoints2);
    end
    if isreal(sol(3))
    Fc = F1 + sol(1)*F2;
    inlierc = findInlierCount(Fc,matchedPoints1,matchedPoints2);
    end
    [val ind] = max([inliera inlierb inlierc]);
    if ind ==1
        F = Fa;
    elseif ind == 2
        F = Fb;
...
Niter = log(1-0.99)/log(1-power(1-eps,7))
iter = iter + 1
end

function [F1,F2,sol] = findFundamentalMatrix(x1,x2,x3,x4,x5,x6,x7,x1n,x2n,x3n,x4n,x5n,x6n,x7n)
syms lam
center = (x1+x2+x3+x4+x5+x6+x7)/7;
maxi = [0,0];mini = [0,0];
maxi(1) = max([x1(1),x2(1),x3(1),x4(1),x5(1),x6(1),x7(1)]);
maxi(2) = max([x1(2),x2(2),x3(2),x4(2),x5(2),x6(2),x7(2)]);
mini(1) = min([x1(1),x2(1),x3(1),x4(1),x5(1),x6(1),x7(1)]);
mini(2) = min([x1(2),x2(2),x3(2),x4(2),x5(2),x6(2),x7(2)]);
scale = (maxi-mini);
T = [1/scale(1),0,(-center(1)/scale(1));0,1/scale(2),(-center(2)/scale(2));0,0,1];
x1 = T*x1'; x2 = T*x2'; x3 = T*x3'; x4 = T*x4'; x5 = T*x5'; x6 = T*x6'; x7 = T*x7';

%Finding Tn
center = (x1n+x2n+x3n+x4n+x5n+x6n+x7n)/7;
maxi = [0,0];mini = [0,0];
maxi(1) = max([x1n(1),x2n(1),x3n(1),x4n(1),x5n(1),x6n(1),x7n(1)]);
maxi(2) = max([x1n(2),x2n(2),x3n(2),x4n(2),x5n(2),x6n(2),x7n(2)]);
mini(1) = min([x1n(1),x2n(1),x3n(1),x4n(1),x5n(1),x6n(1),x7n(1)]);
mini(2) = min([x1n(2),x2n(2),x3n(2),x4n(2),x5n(2),x6n(2),x7n(2)]);
scale = (maxi-mini);
Tn = [1/scale(1),0,(-center(1)/scale(1));0,1/scale(2),(-center(2)/scale(2));0,0,1];
x1n = T*x1n'; x2n = T*x2n'; x3n = T*x3n'; x4n = T*x4n'; x5n = T*x5n'; x6n = T*x6n'; x7n = T*x7n';
